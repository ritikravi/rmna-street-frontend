...
import AccessoryFilters from '../components/product/AccessoryFilters';
...
export default function WomenAccessoriesPage() {
  ...
  const pageTitle = SUBCATEGORY_LABELS[filters.subcategory] || 'Women Accessories';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {Object.entries(SUBCATEGORY_LABELS).map(([val, label]) => (
          <button
            key={val}
            onClick={() => handleFilterChange({ subcategory: val })}
      ...
      <div className="flex gap-8">
        ...
        <div className="hidden md:block w-56 flex-shrink-0">
          <AccessoryFilters filters={filters} onChange={handleFilterChange} />
        </div>
        ...
        {showFilters && (
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            <div className="absolute right-0 top-0 h-full w-72 bg-white p-6 overflow-y-auto">
              ...
              <AccessoryFilters filters={filters} onChange={(u) => { handleFilterChange(u); setShowFilters(false); }} />
            </div>
          </div>
        )}
        ...
        <div className="flex-1">
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="animate-pulse">
                  <div className="bg-zinc-200 aspect-square" />
            ...
            <div className="text-center py-20">
              <p className="text-zinc-500 text-lg">No accessories found</p>
              <button
                onClick={() => handleFilterChange({ subcategory: '', minPrice: '', maxPrice: '' })}
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                {items.map((p) => <AccessoryCard key={p._id} product={p} />)}
              </div>
              {pages > 1 && (
